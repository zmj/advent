...
func solve(r io.Reader, w io.Writer) error {
	...
	for id, g := range guards {
		...
		sleepMaxMinute := -1
		sleepMaxMinuteTotal := -1
		for min, total := range sleepMinutes {
			if total > sleepMaxMinuteTotal {
				sleepMaxMinute = min
				sleepMaxMinuteTotal = total
			}
		}
		if sleepMaxMinuteTotal > maxSleepMaxMinuteTotal {
			maxSleepGuardID = id
			maxSleepMaxMinute = sleepMaxMinute
			maxSleepMaxMinuteTotal = sleepMaxMinuteTotal
		}
	}
	_, err := fmt.Fprintf(w, "%v", maxSleepGuardID*maxSleepMaxMinute)
	return err
}
