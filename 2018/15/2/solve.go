...
func solve(r io.Reader, w io.Writer) error {
	...
	basegame, err := parseGame(scanner)
	...
	var requiredElves int
	for _, u := range basegame.units {
		if u.name == 'E' {
			requiredElves++
		}
	}
	var outcome int
	for elfAP := 3; ; elfAP++ {
		subgame := basegame.copy()
		for _, u := range subgame.units {
			if u.name == 'E' {
				u.ap = elfAP
			}
		}
...
func (g game) copy() game {
	var h game
	for _, grow := range g.grid {
		hrow := make([]loc, len(grow))
		copy(hrow, grow)
		h.grid = append(h.grid, hrow)
	}
	for _, gu := range g.units {
		hu := &unit{gu.name, gu.x, gu.y, gu.hp, gu.ap}
		h.grid[hu.y][hu.x].unit = hu
		h.units = append(h.units, hu)
	}
	return h
}
