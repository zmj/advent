...
func solve(r io.Reader, w io.Writer) error {
	...
	for tick := 0; ; tick++ {
		...
		for _, cart := range grid.carts {
			...
			if ok {
				...
				cart.crashed = true
				loc := &grid.tracks[cart.y][cart.x]
				delete(safeCarts, loc.cart)
				loc.cart.crashed = true
				loc.cart = nil
			}
		}
		grid.carts = nil
		for c := range safeCarts {
			grid.carts = append(grid.carts, c)
		}
		if len(grid.carts) == 1 {
			survivor = grid.carts[0]
			break
		}
	}
	...
	_, err = fmt.Fprintf(w, "%v,%v", survivor.x, survivor.y)
	return err
}
